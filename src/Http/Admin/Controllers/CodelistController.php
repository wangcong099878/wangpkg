...
use App\Admin\Repositories\Codelist; //App\Admin\Repositories\User
use Dcat\Admin\Form;
use Dcat\Admin\Grid;
...
class CodelistController extends AdminController
{
    ...
    {
        return Grid::make(new Codelist(), function (Grid $grid) {
            ...
            $grid->column('ip', 'ip');
            $grid->column('scene', '业务场景')->using(\App\Models\Codelist::$sceneMap);
    ...
    {
        return Show::make($id, new Codelist(), function (Show $show) {
            // 这里的字段会自动使用翻译文件
            /*$show->field('id');
            $show->field('name');
            $show->field('email');
    ...
    {
        return Form::make(new Codelist(), function (Form $form) {
            ...
            $form->saved(function (Form $form) {
                return $form->response()->success('保存成功')->redirect('codelist');
            });
            ...
            /*            $form->saving(function (Form $form) {
                            ...
                            //可以中断用户请求
                            return $form->response()->error('系统错误')->redirect('codelist');
                        });*/
