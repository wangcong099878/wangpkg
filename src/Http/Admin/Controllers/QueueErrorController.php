...
use App\Admin\Repositories\QueueError; //App\Admin\Repositories\User
use Dcat\Admin\Form;
use Dcat\Admin\Grid;
...
class QueueErrorController extends AdminController
{
    ...
    {
        return Grid::make(new QueueError(), function (Grid $grid) {
            ...
            $grid->column('try_again', '是否重试')->using(\App\Models\QueueError::$tryAgainMap);
    ...
    {
        return Show::make($id, new QueueError(), function (Show $show) {
            // 这里的字段会自动使用翻译文件
            /*$show->field('id');
            $show->field('name');
            $show->field('email');
            ...
            $show->field('error_reason', '执行信息');
            $show->field('try_again', '是否重试');
    ...
    {
        return Form::make(new QueueError(), function (Form $form) {


            $form->text('taskname', '队列名称');
            $form->text('ulid', 'ulid');
            $form->textarea('error_reason', '执行信息');
            $form->text('try_again', '是否重试')->default(2);
            $form->text('error_num', '错误次数');

            $form->saved(function (Form $form) {
                return $form->response()->success('保存成功')->redirect('queueError');
            });
            ...
            /*            $form->saving(function (Form $form) {
                            ...
                            //可以中断用户请求
                            return $form->response()->error('系统错误')->redirect('queueError');
                        });*/
